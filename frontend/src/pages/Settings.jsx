...
import { Loader2, Mail, Server, Lock, CheckCircle, AlertCircle, MessageCircle, Send, Key, Plus, Trash2, BookOpen, Link, Edit, ToggleLeft, ToggleRight, ChevronDown, ChevronUp, Database, AlertTriangle, Download, RefreshCw, FileJson, Upload, UploadCloud } from 'lucide-react';
...
export const Settings = () => {
  ...
  const handleRestoreDatabase = async () => {
    ...
    const confirmMsg = restoreMode === 'replace'
      ? 'FULL RESTORE: This will REPLACE all data in matching collections. Are you sure?'
      : 'MERGE RESTORE: This will add missing records without deleting existing data. Continue?';
    if (!window.confirm(confirmMsg)) return;

    setDbAction('restore');
    try {
      const apiFn = restoreMode === 'replace' ? databaseAPI.restoreDatabaseReplace : databaseAPI.restoreDatabase;
      const res = await apiFn(restoreFile);
      const d = res.data;
      ...
      {isAdmin && (
        <Card className="border-red-200" data-testid="database-management-card">
          ...
          <CardContent className="space-y-6">
            ...
            {/* Log Cleanup */}
            <div className="space-y-3">
              <h3 className="text-sm font-semibold text-slate-700 border-b pb-2">Restore Database</h3>
              <Alert className="bg-blue-50 border-blue-200">
                <Database className="h-4 w-4 text-blue-600" />
                <AlertDescription className="text-blue-800 text-xs">
                  Upload a JSON backup file to restore data. <strong>Merge</strong> adds missing records only. <strong>Replace</strong> overwrites all data in each collection (users & settings are always merged safely).
                </AlertDescription>
              </Alert>
              <div className="space-y-3">
                <div className="flex gap-2 items-center">
                  <label className="flex-1 relative">
                    ...
                    />
                    <div className="flex items-center gap-2 px-3 py-2 border rounded-md bg-white hover:bg-slate-50 cursor-pointer text-sm">
                      <Upload className="w-4 h-4 text-slate-500" />
                      <span className="text-slate-600 truncate">{restoreFile ? restoreFile.name : 'Choose backup file (.json)'}</span>
                    </div>
                  </label>
                </div>
                <div className="flex gap-2 items-center">
                  <div className="flex gap-1 bg-slate-100 rounded-md p-0.5">
                    ...
                    <Button
                      variant={restoreMode === 'replace' ? 'destructive' : 'ghost'}
                      size="sm"
                  ...
                  <Button
                    ...
                    disabled={!restoreFile || dbAction === 'restore'}
                    size="sm"
                    variant={restoreMode === 'replace' ? 'destructive' : 'default'}
                    data-testid="restore-database-btn"
                  >
                    {dbAction === 'restore' ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : <UploadCloud className="w-4 h-4 mr-1" />}
                    Restore
                  </Button>
                </div>
              </div>
