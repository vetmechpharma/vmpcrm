    ...
    <div className={className} data-testid="offers-carousel">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-slate-800 text-sm">Special Offers</h3>
          <span className="text-[11px] px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 font-medium">{offers.length} products</span>
        </div>
      ...
      <div ref={scrollRef} className="flex gap-3 overflow-x-auto pb-2" style={{ scrollSnapType: 'x mandatory', scrollbarWidth: 'none', msOverflowStyle: 'none', WebkitOverflowScrolling: 'touch' }}>
        <style>{`.offers-scroll::-webkit-scrollbar{display:none}`}</style>
        {offers.map((item) => (
          <div key={item.id}
            ...
            {item.image_url ? (
              ...
              <div className="h-28 flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #fffbeb, #fef3c7)' }}>
                <span className="text-3xl font-bold text-amber-200">{item.item_name.charAt(0)}</span>
              </div>
            )}
            ...
            <div className="p-3">
              ...
              {/* Offer badge */}
              {item.offer && (
                <div className="flex items-start gap-1.5 mb-1.5 p-2 rounded-lg bg-amber-50 border border-amber-200">
                  <Tag className="w-3 h-3 text-amber-600 mt-0.5 shrink-0" />
                  <p className="text-xs font-semibold text-amber-800 leading-tight">{item.offer}</p>
                </div>
              )}
